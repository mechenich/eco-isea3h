...
# -----------------------------------------------------------------------------
resolution = 9
attributefield = "DOMSOI"

workingpath = "%s/Ecosphere/ISEA3H%02i/DSMW_V36/Working" % (z, resolution)

datasetpath = "%s/Ecosphere/Datasets/DSMW_V36" % z
...
for featureindex in range(hexagonfile.numRecords):
    feature = hexagonfile.record(featureindex)
...
for intersectfile in intersectfiles:
    ...
    if os.path.exists(fractionsfile) == False:
        ...
        for featureindex in range(datasetfile.numRecords):
            feature = datasetfile.record(featureindex)
        ...
        for featureindex in range(intersectfile.numRecords):
            feature = intersectfile.record(featureindex)
            hid = feature[hidindex]
            attribute = feature[attributeindex]
